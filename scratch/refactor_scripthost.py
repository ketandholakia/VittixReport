...
"""
    ...
    for i in range(1, len(parts)):
        ...
        if match:
            ...
            # If the original block had "Exit;" in it (other than the end), it's fine.
            body = body.replace("Result.", "AResult.")
            body = body.replace("Result :=", "AResult :=")
            
            # fix missing 'end;' if any due to regex greediness inside the part
            # Wait, the part split guarantees there are no other 'if Key =' blocks in it.
            # But the match regex ends at the last "Exit; end;" in the part.
