        ...
        if "function TReportScriptHostAdapter.ExecuteSingleBeforeObject" in line:
            ...
            while i < len(lines):
                if lines[i].strip() == "Result.Handled := True;" and "ScriptUnsupported[UnknownCommand]" in lines[i+2]:
                    ...
                    # skip the rest of the old method
                    while i < len(lines):
                ...
                if "if Key = '" in lines[i]:
                    ...
                    # Fix anchorright missing else block
                    if key == "anchorright" or key == "anchorbottom":
                        # The original code has a bug where `else if SameText(Value, 'False') then B := False`
                        # is immediately followed by `Result.Unsupported := True` without `else begin`.
                        # Let's fix this in block_lines by checking for this sequence.
                        for j in range(len(block_lines)):
                            if "B := False" in block_lines[j] and j+1 < len(block_lines) and "Result.Unsupported" in block_lines[j+1]:
                                ...
                                # we need an end; before the AObject assignment.
