...
""")
        ...
        if "function TReportScriptHostAdapter.ExecuteSingleBeforeObject" in line:
            ...
            for key, block in handlers:
                ...
                out_lines.append("    const AValue, AScript: string;\n")
                ...
                out_lines.append("  begin\n")
                for bl in block:
                    out_lines.append(bl.replace("Result.", "AResult.").replace("Result :=", "AResult :=").replace("SameText(Value,", "System.SysUtils.SameText(AValue,").replace(", Value", ", AValue").replace(" Value;", " AValue;"))
                out_lines.append("  end);\n\n")
