...
- Confirm `34_reportdata_contract.vrt` renders executed-report rows through the `ReportData` alias and a supplied filter summary parameter.
...
- Demo host parser supports `AnchorBottom := False|True`.
...
- Demo host parser supports `FontBold := False|True` for `TReportTextObject`.
- Demo host parser supports `FontItalic := False|True` for `TReportTextObject`.
...
- Demo host parser supports `DataField := <string>` for `TReportImageObject`.
...
- Demo host parser supports `PaddingTop := <integer>` for `TReportTextObject`.
- Demo host parser supports `PaddingRight := <integer>` for `TReportTextObject`.
...
- Demo host parser supports `BackgroundCondition := <string>` for `TReportTextObject`.
- Demo host parser supports `BorderColorCondition := <string>` for `TReportTextObject`.
...
- Runtime Event Callback Demo subtest `AnchorBottom := True` reports PASS.
...
- Runtime Event Callback Demo subtest `FontBold := True` reports PASS.
- Runtime Event Callback Demo subtest `FontItalic := True` reports PASS.
...
- Runtime Event Callback Demo subtest `DataField := ImagePath` reports PASS.
...
- Runtime Event Callback Demo subtest `PaddingTop := 7` reports PASS.
- Runtime Event Callback Demo subtest `PaddingRight := 9` reports PASS.
...
- Runtime Event Callback Demo subtest `BackgroundCondition := Value > 0` reports PASS.
- Runtime Event Callback Demo subtest `BorderColorCondition := Value < 100` reports PASS.
...
- Runtime Event Callback Demo subtest `Text := 'A;B'; Foo := 1` reports quoted-semicolon + unsupported PASS.
- Runtime Event Callback Demo subtest on non-text object `Text := 'X'; Background := clYellow` reports object-type mismatch PASS.
...
- Quote-aware semicolon + unsupported mix keeps `A;B` as one literal and still reports following `UnknownCommand`.
- Object-type mismatch subtest reports `ScriptUnsupported[ObjectType]` for text-only commands on non-text objects.
...
- Mixed valid+invalid sequence confirms left-to-right behavior: valid commands apply while unsupported commands are still tagged and counted.
