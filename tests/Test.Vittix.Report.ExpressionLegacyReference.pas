unit Test.Vittix.Report.ExpressionLegacyReference;

{
  Phase 4B-1 — FROZEN LEGACY REFERENCE IMPLEMENTATION (test-only).

  This unit is a verbatim copy of the TReportExpression.Evaluate algorithm as
  it existed before the Phase 4B-1 compatibility evaluator replacement.  It
  exists for one purpose only: the differential compatibility harness in
  Test.Vittix.Report.Phase4B evaluates the same expression with this reference
  and with the production compatibility evaluator and compares Variant type,
  Variant value and failure behavior.

  Rules for this file:
    - It is NOT part of the runtime package
      (packages/VittixReportRuntime.dpk) and must never be linked into a
      shipped build.
    - It must never be "fixed", modernized or refactored.  Any change here
      invalidates the differential evidence.
    - It delegates aggregates to the same TReportAggregates.TryEvaluate as
      production, because the inner-expression evaluation inside an aggregate
      always uses the production evaluator (the aggregate subsystem is a
...
type
  TReportLegacyReferenceExpression = class
  public
...
// ---------------------------------------------------------------------------
// System token resolver (verbatim legacy)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Field + system token replacer (verbatim legacy)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Simple arithmetic parser (verbatim legacy)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Main Evaluate (verbatim legacy)
// ---------------------------------------------------------------------------

class function TReportLegacyReferenceExpression.Evaluate(
  const Expr: string;
  const Context: TExpressionContext): Variant;
var
  S:        string;
...
begin
  ...
  // Step 4 — comparison operators (must be checked before quoted-string
  // literal so that expressions such as 'hello' = 'hello' are evaluated as
  // comparisons)
